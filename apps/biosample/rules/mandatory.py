...
class BS_R0020(BsRule):
    ...
    def validate(self, submission, context):
        out = []
        for rec in submission.records:
            if _is_empty(rec.organism):
                out.append(self.result(sample=(rec.sample_name or rec.accession), message=self.description))
        return out
...
class BS_R0027(BsRule):
    ...
    def validate(self, submission, context):
        out = []
        for rec in submission.records:
            if _is_empty(rec.package) or context.package_def(rec.package) is None:
            ...
            for name in sorted(mandatory):
                if name in desc_fields:
                    if not _is_empty(desc_fields[name]) or not _is_empty(rec.attr(name)):
                        continue
                    missing.append(name)
                elif _is_empty(rec.attr(name)):
                    missing.append(name)
            if missing:
