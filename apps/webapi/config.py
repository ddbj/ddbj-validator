...
# web サーバのログファイル（コンテナ stdout に加えてここにも出す）。
# コンテナを消しても残り、log driver の設定に依存せず traceback を追えるようにするため。
WEB_LOG = os.environ.get("DDBJ_WEB_LOG", str(Path(DATA_DIR) / "web.log"))
