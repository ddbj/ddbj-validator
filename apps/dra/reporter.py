"""DRA validator のレポート出力（TSV サマリ/詳細 ＋ -j で JSON）。bioproject/biosample と同形式。"""
import json
...
def build_summary(results, counts_obj, fname, version, when, elapsed):
    ...
    return (
        "=== DRA Validation Summary ===\n"
        f"Validation Date: {when}\nProcess Time: {elapsed}\nVersion: {version}\n"
        f"Input: {fname}\n"
        f"Experiments: {counts_obj.get('experiments',0)}   Runs: {counts_obj.get('runs',0)}   "
