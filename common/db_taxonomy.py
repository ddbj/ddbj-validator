...
def get_tax_group(org_name, lineage):
    ...
    is_environmental = (
        "unclassified sequences" in lineage_lower
        or "environmental samples" in lineage_lower
        or "uncultured" in org_lower
        or "environmental" in org_lower
    )
        ...
        try:
            ...
            for org, u_id in pending_recursive_check.items():
                if u_id in valid_ut_ids:
                    ...
                    best_match = sorted(temp_results[org.lower()], key=lambda x: x[0])[
                        0
                    ]
