...
直接 `docker run` で実行する場合の基本構造は以下の通りです。`ann` と `fasta` ファイルが存在するカレントディレクトリをコンテナ内の `/data` にマウントして実行します。
...
NCBI API を利用し、結果を output フォルダに出力する場合：
```bash
# macOS/Unix
...
* `validation_report_summary.txt`: error や warning のサマリー（ルールごとの発生件数）
* `validation_report_details.txt`: error や warning が発生した行番号やメッセージの全リスト
* `fixed/` 承認された Autofix による修正が施されたアノテーションおよび FASTA ファイルが出力されるディレクトリ
* `aa/` CDS feature から翻訳されたアミノ酸配列（FASTA 形式）が出力されるディレクトリ
